...
class IMAPClusterManager:
    ...
    async def start_cluster(self) -> None:
        ...
        try:
            ...
            accounts = await self.db_manager.get_active_accounts()
            ...
            logger.info(f"IMAP cluster started with {len(self.worker_processes)} workers")
    ...
    async def add_account(self, account: AccountConfig) -> bool:
        ...
        try:
            # Add to database
            await self.db_manager.add_account(account)
    ...
    async def get_cluster_stats(self) -> dict[str, int | list[int]]:
        ...
        stats: dict[str, int | list[int]] = {
            ...
            "dead_workers": len(self.worker_processes) - len(alive_workers),
